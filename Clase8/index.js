...
let plantillaNoticias = (titulo, imgUrl, descripcion, fecha, tipoNacional) => {
    return `<div class="noticia">
                <img src="${imgUrl}" alt="" class="imgNoticia">
                <h3>${titulo}</h3>
                <p>${descripcion}</p>
            </div>`
}

//Reemplazar el contenido de span con el texto de la variable

document.querySelector('#noticia1-1').innerHTML = plantillaNoticias(noticias[0].titulo, noticias[0].imgUrl, noticias[0].descripcion, noticias[0].fecha, noticias[0].tipoNacional);
document.querySelector('#noticia1-2').innerHTML = plantillaNoticias(noticias[1].titulo, noticias[1].imgUrl, noticias[1].descripcion, noticias[1].fecha, noticias[1].tipoNacional);
document.querySelector('#noticia1-3').innerHTML = plantillaNoticias(noticias[2].titulo, noticias[2].imgUrl, noticias[2].descripcion, noticias[2].fecha, noticias[2].tipoNacional);
